...
- A zrok account token (from `zrok2 admin create account` on a self-hosted instance, or from the zrok console on a hosted instance)
...
This creates `~/.zrok2/environment.json`. Verify with:

```bash
ls ~/.zrok2/environment.json
```
