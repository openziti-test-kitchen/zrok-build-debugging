...
To accomplish this, we're going to run 3 separate `zrok2 access private` commands on our VPS (see the [Agent guide](../agent/index.mdx) and the [Linux Agent Service](../agent/linux-service.mdx) for details on setting this up). One command each for shares `A`, `B`, and `C`. The `zrok2 access private` command works like this:
