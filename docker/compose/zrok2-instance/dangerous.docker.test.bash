...
teardown() {
    ...
    # Include --profile metrics to also remove metrics containers and volumes
    # that may have been started by a prior run with --keep.
    if ! (cd "${COMPOSE_PROJECT_DIR}" && docker compose --profile metrics down -v 2>/dev/null); then
        true  # best-effort
    fi
}
