...
step_database_sqlite3() {
    if ! dpkg -l libsqlite3-0 &>/dev/null; then
...
step_start_metrics_bridge() {
    ...
    # Make the file readable by the metrics user (via ziti-controller group or world-readable)
    chmod o+r "$FABRIC_USAGE_PATH"

    # Grant read access to the ctrl.yml for the metrics bridge user
    if getent passwd zrok2-metrics &>/dev/null; then
        usermod -aG zrok2-controller zrok2-metrics 2>/dev/null || true
    fi
